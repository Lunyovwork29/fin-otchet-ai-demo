  ...
  sections: [
    {
      ...
      children: [
        ...
        bullet(
          `Задвоенный платёж — операция «${data.duplicate_error.purpose}» ` +
          `(${fmt(data.duplicate_error.amount)}) встречается в таблице ${data.duplicate_error.occurrences} раза ` +
          "с одинаковой датой, контрагентом, назначением и суммой. ИИ-сервис сравнил все операции " +
          "и обнаружил полностью идентичные строки — лишний платёж исключён из отчётности как ошибочный."
        ),
        bullet(
          `Сумма не соответствует назначению платежа — в операции «${data.mismatch_error.purpose}» ` +
          `указана сумма ${fmt(data.mismatch_error.actual)}, хотя расчёт по тексту самого назначения ` +
          `даёт ${fmt(data.mismatch_error.expected)}. Расхождение ${fmt(data.mismatch_error.diff)} было найдено ` +
